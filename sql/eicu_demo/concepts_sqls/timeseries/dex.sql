DROP TABLE IF EXISTS eicu_demo_coh_concepts.dex;
CREATE TABLE eicu_demo_coh_concepts.dex AS
SELECT
    cw.uniquepid,
...
WHERE LOWER(src.drugname) ~*'(d50|dextrose.+50 ?%)'
...
WHERE LOWER(src.drugname) ~*'(d10|dextrose.+10 ?%).+ml/hr'
AND src.drugdosage IS NOT NULL;
