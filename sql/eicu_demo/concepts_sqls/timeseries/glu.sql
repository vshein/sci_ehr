DROP TABLE IF EXISTS eicu_demo_coh_concepts.glu;
CREATE TABLE eicu_demo_coh_concepts.glu AS
SELECT
    cw.uniquepid,
...
WHERE LOWER(src.labname) ~ 'glucose'
...
WHERE LOWER(src.labname) ~ 'bedside glucose'
AND src.labresult IS NOT NULL;
